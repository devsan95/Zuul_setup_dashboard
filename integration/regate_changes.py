...
def run(gerrit_info_path, root_change_no):
    ...
    change_info = integration_change.IntegrationChange(rest, root_change_no)
    if change_info.get_feature_id():
        root_change_number = change_info.get_root_change()
        root_change = integration_change.RootChange(rest, root_change_number)
