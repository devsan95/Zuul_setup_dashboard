...
def main(title, content, author, alert_type, icon, label, label_type,
    ...
    zuul_server_container = docker_client.containers.get(zuul_server_name)
    # copy list.yaml from docker container
    bist, stat = zuul_server_container.get_archive("/ephemeral/zuul/www/notification/list.yaml")
    data = io.BytesIO(bist.read())
    tarobj = tarfile.open(fileobj=data)
    # path to cwd is not a good choose, better create a folder
    ...
    if gerrit_available:
        ...
        # create a tar file
        notification_tarstream = io.BytesIO()
        notification_tar = tarfile.TarFile(fileobj=notification_tarstream, mode='w')
        add_file_to_tar(tar_file=notification_tar,
                        added_file_name="list.yaml",
                        data_str=list_save_yaml)
        add_file_to_tar(tar_file=notification_tar,
                        added_file_name="index.html",
                        data_str=output)
        add_file_to_tar(tar_file=notification_tar,
                        added_file_name="history.html",
                        data_str=history_str)

        notification_tar.close()
        notification_tarstream.seek(0)

        zuul_server_container.put_archive(path='/ephemeral/zuul/www/notification/',
                                          data=notification_tarstream)
        print("DEBUG_INFO: main: Copy files into container success!")


def add_file_to_tar(data_str, tar_file, added_file_name):
    print("DEBUG_INFO: add_file_to_tar: {} start!".format(added_file_name))
    file_data = data_str.encode('ascii')
    tarinfo = tarfile.TarInfo(name=added_file_name)
    tarinfo.size = len(file_data)
    tarinfo.mtime = time.time()
    tar_file.addfile(tarinfo, io.BytesIO(file_data))
    print("DEBUG_INFO: add_file_to_tar: {} end!".format(added_file_name))
