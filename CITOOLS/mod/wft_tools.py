...
def get_staged_from_wft(wft_name, component=None, project=None):
    ...
    if 'bb_commit' not in build_content or 'bb_location' not in build_content or 'bb_type' not in build_content:
        return {}
    return {
        'location': build_content['bb_location'].encode('utf-8') if build_content['bb_location'] else build_content['bb_location'],
        'type': build_content['bb_type'].encode('utf-8') if build_content['bb_type'] else build_content['bb_type'],
        'commit': build_content['bb_commit'].encode('utf-8') if build_content['bb_commit'] else build_content['bb_commit']
    }
